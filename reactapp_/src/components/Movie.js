...
function Movie(props) {
  ...
  const handleClick = async () => {
    ...
    if (!likeMovie){
      props.handleClickParent(!likeMovie, props.movieName, props.movieImg);
      await fetch('/wishlist-movie', {
        method: 'POST',
        headers: {'Content-Type':'application/x-www-form-urlencoded'},
        body:`name=${props.movieName}&image=${props.movieImg}`
      })
    } else {
      props.handleClickDeleteMovieParent(props.movieName)
      await fetch(`/wishlist-movie/${props.movieName}`,
      {
        method: 'DELETE'
      });
    }
  ...
  return (
    ...
    <CardBody>
        <p>Like <FontAwesomeIcon style={colorLike} icon={faHeart} onClick={()=> handleClick()}/></p>
